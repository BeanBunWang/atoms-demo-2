...
const REQUIRED_SOURCE_PATHS = ["src/App.jsx", "src/styles.css", "src/app.config.json"];
...
function normalizeSourcePath(path) {
  const value = clean(path, "", 96);
  if (!value || value.startsWith("/") || value.includes("..") || !/^[\w./-]+$/.test(value)) return "";
  return value;
}

function createAppSource(preview) {
  const sections = (preview.sections || [])
    .map((section) => `        <Section key="${section.id}" type="${section.type}" title="${section.title}" description="${section.description || ""}" metrics={${JSON.stringify(section.metrics || [])}} items={${JSON.stringify(section.items || [])}} />`)
    .join("\n");
  const interactive = preview.appType === "calculator"
    ? "      <Calculator capabilities={ATOMS_PREVIEW_CONFIG.capabilities} onCalculate={saveHistory} />"
    : preview.appType === "snake"
      ? "      <SnakeGame capabilities={ATOMS_PREVIEW_CONFIG.capabilities} />"
      : "";
...
export default function App() {
  return (
    <ProductShell
      accent={ATOMS_PREVIEW_CONFIG.accent}
      background={ATOMS_PREVIEW_CONFIG.background}
      data-theme={ATOMS_PREVIEW_CONFIG.themeId || "custom"}
      data-heading={ATOMS_PREVIEW_CONFIG.headingStyle || "editorial"}
      template={ATOMS_PREVIEW_CONFIG.template}
    >
...
function createStylesSource(preview) {
  ...
  --atoms-background: ${preview.background};
  --atoms-text: #181a20;
  --atoms-muted: #656b78;
}
...
.hero__action,
.section-card strong {
  color: var(--atoms-accent);
}
`;
}

function createConfigSource(preview) {
  return `${safeJson({ schemaVersion: 2, preview })}\n`;
}

export function createSourceFiles(preview, sourceFiles = []) {
  const normalized = list(sourceFiles, 12)
    .map((file) => {
      const path = normalizeSourcePath(file?.path);
      const content = typeof file?.content === "string" ? file.content.slice(0, 24000) : "";
      if (!path || !content) return null;
      return { path, language: clean(file?.language, inferLanguage(path), 24), content };
    })
    .filter(Boolean);
  const fallbackRequired = [
    { path: "src/App.jsx", language: "jsx", content: createAppSource(preview) },
    { path: "src/styles.css", language: "css", content: createStylesSource(preview) },
    { path: "src/app.config.json", language: "json", content: createConfigSource(preview) }
  ];
  const previewJson = JSON.stringify(preview);
  const appMatches = (file) => {
    const marker = file?.content?.match(/ATOMS_CONFIG_START \*\/\s*([\s\S]*?)\s*\/\* ATOMS_CONFIG_END/);
    if (!marker) return false;
    try { return JSON.stringify(JSON.parse(marker[1])) === previewJson; } catch { return false; }
  };
  const configMatches = (file) => {
    try {
      const parsed = JSON.parse(file?.content || "");
      return JSON.stringify(parsed.preview || parsed) === previewJson;
    } catch {
      return false;
    }
  };
  const cssIsSafe = (file) => !/@import\b|url\s*\(|expression\s*\(|javascript\s*:|\bbehavior\s*:/i.test(file?.content || "");
  const byPath = new Map(normalized.map((file) => [file.path, file]));
  for (const fallback of fallbackRequired) {
    const candidate = byPath.get(fallback.path);
    const valid = Boolean(candidate) && (fallback.path === "src/App.jsx"
      ? appMatches(candidate)
      : fallback.path === "src/app.config.json"
        ? configMatches(candidate)
        : cssIsSafe(candidate));
    if (!valid) byPath.set(fallback.path, fallback);
  }
  return [...byPath.values()];
}

function extractPreviewConfig(sourceFiles) {
  const app = sourceFiles?.find((file) => file.path === "src/App.jsx");
  const marker = app?.content?.match(/ATOMS_CONFIG_START \*\/\s*([\s\S]*?)\s*\/\* ATOMS_CONFIG_END/);
...
export function normalizeArtifact(value, intent, { previousPreview, baseRevision = 0 } = {}) {
  ...
  const candidate = updating ? applyPreviewPatch(previousPreview, value?.patch || {}) : value?.preview || {};
  const raw = candidate;
  ...
  const sourceFiles = createSourceFiles(preview, value?.sourceFiles);
  return {
    ...
    files: [...new Set([...(list(value?.files, 7).length ? list(value?.files, 7) : []), ...sourceFiles.map((file) => file.path)])].map((item) => clean(item, "src/App.jsx", 70)),
    sourceFiles
  };
}
...
export function validateArtifact(artifact, intent, { previousPreview, baseRevision } = {}) {
  ...
  for (const path of REQUIRED_SOURCE_PATHS) if (!sourcePaths.has(path)) issues.push(`缺少源码文件 ${path}`);
  if (sourceFiles.some((file) => !file.language || typeof file.content !== "string" || !file.content.trim())) issues.push("源码文件缺少语言或内容");
  if (sourceFiles.some((file) => file.path.endsWith(".css") && /@import\b|url\s*\(|expression\s*\(|javascript\s*:|\bbehavior\s*:/i.test(file.content))) issues.push("源码样式包含不安全的外部或可执行内容");
  const markerPreview = extractPreviewConfig(sourceFiles);
  if (!markerPreview || JSON.stringify(markerPreview) !== JSON.stringify(preview)) issues.push("App.jsx 缺少匹配当前 Preview 的安全 ATOMS_CONFIG JSON 标记");
  const configFile = sourceFiles.find((file) => file.path === "src/app.config.json");
  try {
    const parsedConfig = JSON.parse(configFile?.content || "");
    if (JSON.stringify(parsedConfig.preview || parsedConfig) !== JSON.stringify(preview)) issues.push("app.config.json 与当前 Preview 不一致");
  } catch {
    issues.push("app.config.json 不是有效 JSON");
  }
...
export function createCodeArtifact(preview) {
  return createAppSource(preview);
}
