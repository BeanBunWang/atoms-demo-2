...
> 非官方项目，与 Atoms 无隶属关系。生成源码可查看、编辑、校验和回滚；Preview 只执行受控 UI schema 与领域 runtime，不直接执行任意模型 JavaScript。应用内 Publish 为产品交互模拟。
...
- 提供公开主页、注册、登录、退出和账号状态；登录后可跨刷新恢复云端工作区
...
- Code 工作区展示 `src/App.jsx`、`src/styles.css`、`src/app.config.json`；可直接编辑，应用前校验并创建 revision，支持 Reset、Ask Agent 和版本回滚
- Design 的 Visual Editor 可按 Hero/区块编辑内容与强调色，Library 可添加受控组件，Theme 可应用预置主题；结果同步到 Preview、Code 和当前工作区
